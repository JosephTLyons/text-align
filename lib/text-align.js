...
module.exports = {
    activate() {
        this.commandsDisposable = atom.commands.add('atom-text-editor:not([mini])', {
            'left-align:toggle'() {
                adjustText(atom.workspace.getActiveTextEditor(), alignment.LEFT);
            },

            'center-align:toggle'() {
                adjustText(atom.workspace.getActiveTextEditor(), alignment.CENTER);
            },

            'right-align:toggle'() {
                adjustText(atom.workspace.getActiveTextEditor(), alignment.RIGHT);
            },

            'justify:toggle'() {
                adjustText(atom.workspace.getActiveTextEditor(), alignment.JUSTIFY);
            },
        })
    },
...
function adjustText(editor, alignmentType) {
...
function adjustSelectedLinesOfText(editor, selectionRange, alignmentType) {
    for (let row = selectionRange.start.row; row <= selectionRange.end.row; row++) {
        ...
        let rowString = editor.lineTextForBufferRow(row).trim();

        if (lineIsAcceptableToAdjust(editor.preferredLineLength, rowString.length)) {
            // Replace current line with new text
            editor.setTextInBufferRange([[row, 0], [row, editor.preferredLineLength]],
...
function lineIsAcceptableToAdjust(preferredLineLength, rowStringLength) {
    return (rowStringLength > 0 && (rowStringLength < preferredLineLength));
}

function getAdjustedLine(preferredLineLength, rowString, alignmentType) {
    switch (alignmentType) {
        case alignment.LEFT:
            return rowString;
        case alignment.CENTER:
            return " ".repeat((preferredLineLength - rowString.length) / 2) + rowString;
        case alignment.RIGHT:
            return " ".repeat(preferredLineLength - rowString.length) + rowString;
        case alignment.JUSTIFY:
            return getJustifiedRow(preferredLineLength, rowString);
    }
}
