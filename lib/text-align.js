...
function adjustSelectedLinesOfText(editor, selectionRange, alignmentType) {
    for (let rowNumber = selectionRange.start.row; rowNumber <= selectionRange.end.row; rowNumber++) {
        // Get whitespace-trimmed version of current line's text
        let rowString = editor.lineTextForBufferRow(rowNumber).trim();

        if (lineIsAcceptableToAdjust(editor.preferredLineLength, rowString.length)) {
            // Replace current line with new text
            editor.setTextInBufferRange([[rowNumber, 0], [rowNumber, editor.preferredLineLength]],
