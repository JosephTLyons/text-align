...
function main(editor, alignmentType) {
    ...
    adjustSelectedText(editor, selectionRange, alignmentType);
...
function getJustifiedRow(preferredLineLength, rowString) {
    ...
    for (let i = 0; i < rowWordsArray.length - 1; i++) {
        ...
        if (leftoverSpacesToInsert-- > 0) {
            ...
            // Add extra space to strings starting on right side (- 2 becauase we don't want to add
            // spaces on the right side of the right string in the array)
            if (leftoverSpacesToInsert-- > 0)
